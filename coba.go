...
func main() {
	...
	fmt.Println("Setelah sorting berdasarkan nama:")
	for _, p := range pemainList {
		fmt.Printf("%s (%s tahun) - %s (%s)\n", p.Name, p.Age, p.Team, p.Nation)
	}

	// Search by name
	var cari string
	for cari != "sudah" {
		fmt.Print("Masukkan nama pemain yang ingin dicari: ")
		fmt.Scanln(&cari)

		idx := searchByName(pemainList, cari)
		if idx != -1 {
			fmt.Println("Ditemukan:")
			fmt.Printf("%s (%s), %s - %s\n", pemainList[idx].Name, pemainList[idx].Age, pemainList[idx].Team, pemainList[idx].Nation)
		} else {
